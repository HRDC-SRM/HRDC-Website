// Basic Imports
import React, { useState, useRef } from "react";
import Image from "next/image";

// Components Imports
import EmojiLayerData from "./emoji-list-data.json";
import { Tooltip } from "react-tooltip";

// Types Imports
import { EmojiType } from "@/types/emoji-type";
import ModalView from "@/components/ui-patterns/ModalView";
import Link from "next/link";

const EmojiLayer: React.FunctionComponent<EmojiType> = () => {
  const emojiLayerRef = useRef<Array<EmojiType>>(EmojiLayerData);
  const [highlitablePopupView, setHighlightablePopupView] = useState(false);

  // Duplicate the emoji list for seamless loop
  const emojis = [...emojiLayerRef.current, ...emojiLayerRef.current];

  return (
    <React.Fragment>
      <div className="overflow-hidden w-full select-none relative">
        <div
          className="flex w-max space-x-14 animate-marquee"
        >
          {emojis.map((emoji: EmojiType, emojiIndex: number) => (
            <div key={emojiIndex} className="flex-shrink-0">
              {emoji?.isActive ? (
                <>
                  <button
                    className="w-fit h-fit focus:outline-none"
                    id={`highlitable-tooltip__${emoji?.name}-${emojiIndex}`}
                    data-tooltip-content="🎒 Build Program is now live"
                    onClick={() => setHighlightablePopupView(true)}
                  >
                    <Image
                      src={`/emojis/${emoji?.imageName}`}
                      width={120}
                      height={120}
                      alt={emoji?.name}
                      className={`hover:scale-105 transition-all ${
                        emoji?.isHighlighted &&
                        "hover:brightness-90 animate-bounce"
                      }`}
                      priority
                    />
                    <Tooltip
                      anchorId={`highlitable-tooltip__${emoji?.name}-${emojiIndex}`}
                    />
                  </button>

                  {/* Modal */}
                  <ModalView
                    isOpen={highlitablePopupView}
                    modalViewAction={setHighlightablePopupView}
                  >
                    <h1 className="leading-snug text-lg font-semibold">
                      🎒 Build Program is now live
                    </h1>
                    <p className="leading-6 text-gray-500 font-medium text-sm mt-4 w-[42ch]">
                      The Build Program by HEKORS is now live. The Program
                      will be running for next 1.5 months
                    </p>
                    <div className="my-6 flex flex-col items-start justify-start gap-3">
                      <Link
                        href="https://hekors-build-program.vercel.app/"
                        target="_blank"
                        className="underline font-medium text-product-blue"
                      >
                        → Visit Build Program website
                      </Link>
                    </div>
                  </ModalView>
                </>
              ) : (
                <Image
                  src={`/emojis/${emoji?.imageName}`}
                  width={120}
                  height={120}
                  alt={emoji?.name}
                  className={`hover:scale-105 transition-all max-sm:w-[70px] ${
                    emoji?.isHighlighted && "animate-bounce"
                  }`}
                  priority
                />
              )}
            </div>
          ))}
        </div>
      </div>
    </React.Fragment>
  );
};

export default EmojiLayer;
