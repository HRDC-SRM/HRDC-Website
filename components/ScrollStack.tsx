...
const ScrollStack = ({
  ...
  const updateCardTransforms = useCallback(() => {
    ...
    const stackCompleted = stackCompletedRef.current;
    let hasStackStateChanged = false;
    ...
    cardsRef.current.forEach((card, i) => {
      ...
      if (i === cardsRef.current.length - 1) {
        ...
        const hasPassedCompletion = scrollTop > pinEnd; // Check if we've passed the completion zone
        
        if ((isInView || hasPassedCompletion) && !stackCompleted) {
          stackCompletedRef.current = true;
          hasStackStateChanged = true;
          onStackComplete?.();
        } else if (!isInView && !hasPassedCompletion && stackCompleted) {
